...
{	
    ...
    public function msg()
    {
        if(request()->isAjax()){
            ...
            $user = FormMsg::where($where)->page(input('page'),20)->select();
            $count = FormMsg::where($where)->count();
    ...
    public function msg_save()
    {
        ...
        if(request()->isPost()){
            ...
            if(empty($post['id'])){
                $post['add_time'] = time();
                $oper = $form->allowField(true)->save($post);
            } else {
                $oper = $form->allowField(true)->save($post,['id'=>$id]);
            }
